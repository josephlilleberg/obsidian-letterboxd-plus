...
---
// Sorting
const sorting = ['Film Name', 'Shuffle', 'When Added', 'Release Date', 'Film Length']
...
// Filter pages 
pages = pages.filter(p => p.poster && !p.watched)
...
if (metadata.filterUpcomingReleases) {
	pages = pages.filter(page => page.release_date > new Date())
}
...
// Create the table with only the Poster as a link to the films title
