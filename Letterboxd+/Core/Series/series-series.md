...
for (const p of pages) {
  if (p.poster && statusCounts.hasOwnProperty(p.status)) {
...
let str = ''
if (app.isMobile) {
    str = `<center>:LiCirclePlay: ${statusCounts.Watching} | :LiCirclePause: ${statusCounts.Waiting} | :LiCircleStop: ${statusCounts["On Hold"]} | :LiCheckCheck: ${statusCounts.Completed} | :LiTrash: ${statusCounts.Dropped}</center>`;
} else {
    str = `<center>:LiCirclePlay: <span class="series-overview-label">Watching: ${statusCounts.Watching}</span> | :LiCirclePause: <span class="series-overview-label">Waiting: ${statusCounts.Waiting}</span> | :LiCircleStop: <span class="series-overview-label">On Hold: ${statusCounts["On Hold"]}</span> | \n :LiCheckCheck: <span class="series-overview-label">Completed: ${statusCounts.Completed}</span> | :LiTrash: <span class="series-overview-label">Dropped: ${statusCounts.Dropped}</span></center>`;
}
...
---
// Sorting
const sorting = ['Series Name', 'Shuffle', 'When Added', 'Release Date', 'Rating', 'Rewatch Count']
...
if (context.bound.enableFilters) {
    str = `\`VIEW[Status][text(renderMarkdown)]\` \`INPUT[inlineSelect(option('All', 'All'), option('Watching', 'Watching'), option('Waiting', 'Waiting'), option('On Hold', 'On Hold'), option('Completed', 'Completed'), option('Dropped', 'Dropped')):filterStatus]\``;
}
...
// Fetch pages from the series folder
let pages = dv.pages(`"${folderPath}"`)
    .filter(p => p.poster && p.status !== 'Watchlist');
...
if (metadata.filterSearchQuery) {
    let series = pages.map(f => f.series_name.toLowerCase()); // Normalize for case-insensitivity
    pages = pages.filter(file => 
        file.series_name.toLowerCase().includes(metadata.filterSearchQuery.toLowerCase()) &&
        series.includes(file.series_name.toLowerCase()) // Ensure it exists in the list
    );
}
...
if (metadata.sortOption === 'Series Name') {
	...
	pages = pages.sort(file => file.season_air_dates[0], sortOrder);
...
dv.table(["Poster", "Title"], 
    pages.map(p => [
        `<div class="letterboxd-series-series-${isMobile ? 'mobile' : 'desktop'}-wrapper">
            <a href="${p.file.path}" class="internal-link">
                <img src="${p.poster}" alt="${p.title}" style="border-radius: 5px;">
            </a>
        </div>`,
        `<div class="letterboxd-series-series-${isMobile ? 'mobile' : 'desktop'}-wrapper">
            <div class="letterboxd-series-series-${isMobile ? 'mobile' : 'desktop'}-rating">${getRating(p.rating)}</div>
            <div class="letterboxd-series-series-${isMobile ? 'mobile' : 'desktop'}-like">${getHeartIcon(p)}</div>
        </div>`
    ])
);
```