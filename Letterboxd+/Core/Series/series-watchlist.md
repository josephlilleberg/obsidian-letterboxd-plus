...
---
// Sorting
const sorting = ['Series Name', 'Shuffle', 'When Added', 'Release Date']
...
<!-- toggle: upcoming-releases-->
```meta-bind-js-view
{enableFilters} as enableFilters
---
// Filter: Upcoming Releases Only
let str = '';
if (context.bound.enableFilters) {
	 str = `\`VIEW[Upcoming Releases Only][text(renderMarkdown)]\` \`INPUT[toggle:filterUpcomingReleases]\``;
}
...
actions:
  ...
  - type: updateMetadata
    ...
    value: "Any"
  - type: updateMetadata
    bindTarget: filterUpcomingReleases
    evaluate: false
    value: false
  - type: updateMetadata
...
// Filter: Upcoming Release
if (metadata.filterUpcomingReleases) {
	pages = pages.filter(page => page.season_air_dates[0] > new Date())
}
