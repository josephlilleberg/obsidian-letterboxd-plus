...
---
// Sorting
const sorting = ['Series Name', 'Shuffle', 'When Favorited', 'Release Date', 'Rating', 'Rewatch Count']
...
} else if (context.bound.sortOption === 'When Favorited') {
...
pages = pages.filter(p => p.poster && p.watchlist !== 'Watchlist' && p.favorite)
...
let pages = dv.pages(`"${folderPath}"`); // Use `let` instead of `const`

// Filter out pages without posters and on watchlist
pages = pages.filter(p => p.poster && p.status !== 'Watchlist' && p.favorite)
...
} else if (metadata.sortOption === 'When Favorited') {
	pages = pages.sort(file => file.favorite_date, sortOrder);
