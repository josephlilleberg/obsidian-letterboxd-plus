...
---
// Sorting
const sorting = ['Series Name', 'Shuffle', 'When Liked', 'Release Date', 'Rating', 'Rewatch Count']
...
pages = pages.filter(p => p.poster && p.watchlist !== 'Watchlist')
...
// Filter out pages without posters and on watchlist
pages = pages.filter(p => p.poster && p.status !== 'Watchlist' && p.like)
