...
---
// Sorting
const sorting = ['Series Name', 'Release Date', 'Review Date', 'Review Length']
...
// Filter pages 
pages = pages.filter(p => p.poster && p.status !== 'Watchlist' && p.review && p.review.trim().length > 0)
...
if (rewatchStatus === 'First time watch') {
	pages = pages.filter(file => file.watch_dates && file.watch_dates.length === 0)
...
if (metadata.sortOption === 'Series Name') {
    ...
    pages = pages.sort(file => file.review.length, sortOrder);
}
...
let menus = []; // Initialize the menus array

(async () => {
    for (const page of pages) {
        // Render review markdown into html
        const html = await window.__renderMarkdownToHTML(page.review, app);
        
        // Add a row to the menu for this series
        menus.push([
            `<div class="letterboxd-series-reviews-wrapper">
                <div class="letterboxd-series-reviews-poster">
                    <a href="${page.file.name}" class="internal-link">
                        <img src="${page.poster}" alt="${page.title}">
                    </a>
                </div>
                <div class="letterboxd-series-reviews-content">
                    <div class="letterboxd-series-reviews-header-wrapper">
                        <div class="letterboxd-series-review-title">${page.series_name} <span class="letterboxd-series-review-year">${page.release_year}</span></div>
                    </div>
                    <div class="letterboxd-series-reviews-info-wrapper">
                        <div class="letterboxd-series-reviews-rating">${getRating(page.rating)}</div>
                        <div class="letterboxd-series-reviews-watched"> ${formatDateString(page.review_date)}</div>
                    </div>
                    <div class="letterboxd-series-reviews-content">${html}</div>
    ...
    block.innerHTML = `
      <div class="letterboxd-series-reviews-wrapper">
        ...
        <div class="letterboxd-series-reviews-content">
          ...
          <div class="letterboxd-series-reviews-review">${html}</div>
        </div>
      </div>
    `;
