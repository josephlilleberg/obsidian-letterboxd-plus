...
title: test
description: uj
created_on: 2025-06-29T00:11:42.885
updated: 2025-06-29T00:11:42.885
    ...
    action: {
      ...
      code: `
        (async () => {
            ...
            const series = engine.query.files(file => file.path.startsWith(seriesPath) ? file : undefined);
            ...
            if (listsOption == 'add') {
                ...
                const seriesOptions = series.filter(s => {
                    const seriesFile = app.vault.getFileByPath(s.path);
                    ...
                    const seriesFile = app.vault.getFileByPath(s.path);
                    const frontmatter = app.metadataCache.getFileCache(seriesFile)?.frontmatter;
                    const name = frontmatter?.series_name ?? 'Untitled';

                    return {
                        label: \`\${name}\`,
                        value: {seriesPath: seriesFile.path, listTitle: listTitle, listPath: file.path},
                    };
                });
                ...
                // Let user pick a series to add
                const selectedSeries = await engine.prompt.suggester({
                    placeholder: 'Select a series to add to the list',
                    options: seriesOptions,
                });

                if (!selectedSeries) {
                    new Notice('No series selected.');
                    return;
                }

                // Get selected list metadata
                const selectedSeriesFile = app.vault.getFileByPath(selectedSeries.seriesPath);

                app.fileManager.processFrontMatter(selectedSeriesFile, (frontmatter) => {
                    if (!frontmatter.lists.includes(listTitle)) {
                        frontmatter.lists.push({title: selectedSeries.listTitle, path: selectedSeries.listPath});
                    }
                });
                ...
                const selectedSeries = await engine.prompt.suggester({
                    placeholder: 'Select a series to remove from the list',
                    options: seriesOptions,
                });
                ...
                app.fileManager.processFrontMatter(selectedSeriesFile, (frontmatter) => {
                    frontmatter.lists = (frontmatter.lists || []).filter(
                        l => !(l.title === selectedSeries.listTitle && l.path === selectedSeries.listPath)
                    );
                });

            } else if (listsOption == 'cancel') {
...
---
// Sorting
const sorting = ['Series Name', 'Release Date', 'Rating', 'Rewatch Count']
...
if (context.bound.enableFilters) {
    str = `\`VIEW[Status][text(renderMarkdown)]\` \`INPUT[inlineSelect(option('All', 'All'), option('Watchlist', 'Watchlist'), option('Watching', 'Watching'), option('Waiting', 'Waiting'), option('On Hold', 'On Hold'), option('Completed', 'Completed'), option('Dropped', 'Dropped')):filterStatus]\``;
}
